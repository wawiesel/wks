"""Show the native uninstall command for an MCP client target."""
...
def cmd_uninstall(name: str) -> StageResult:
    """Show the native uninstall command for a supported target.

    Args:
        name: Supported MCP client target name

    Returns:
        StageResult with the native client remove command to run
    """

    def do_work(result_obj: StageResult) -> Iterator[tuple[float, str]]:
        """Resolve the target and report its native uninstall command."""
