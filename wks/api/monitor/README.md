...
**Pydantic for input, schemas for output**: Pydantic models validate configuration input. Command outputs use code-defined output models declared in `__init__.py` and resolved through `output_models.py`. Commands instantiate schema classes (e.g., `MonitorStatusOutput`) and call `.model_dump(mode="python")` to convert to dicts. This keeps validation explicit in Python and removes duplicate schema files.
