...
**Output schemas:** All commands use code-defined Pydantic output models declared in `__init__.py` and resolved through `output_models.py`. Import the schema class (e.g., `ConfigListOutput`), instantiate it with output data, and call `.model_dump(mode="python")` to convert to dict. This keeps output validation explicit in Python while preserving a single command-level contract.
