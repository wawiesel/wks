...
def test_search_semantic_exclude_paths_filters_results(tmp_path, monkeypatch):
    """exclude_paths in index spec removes matching URIs from semantic search results."""
    ...
    junk_doc = excluded_dir / "fission.txt"
    junk_doc.write_text("Nuclear fission products are generated during reactor operation.\n")
    kept_doc = kept_dir / "fission.txt"
    kept_doc.write_text("Nuclear fission products are generated during reactor operation.\n")
    ...
    result = run_cmd(search_cmd, "fission", k=10)
    assert result.success is True
    hit_uris = [h["uri"] for h in result.output["hits"]]
    assert not any(str(excluded_dir) in u for u in hit_uris), "excluded path appeared in results"
    assert any(str(kept_dir) in u for u in hit_uris), "kept path missing from results"


def test_search_lexical_exclude_paths_filters_results(tmp_path, monkeypatch):
    """exclude_paths in index spec removes matching URIs from lexical search results."""
    excluded_dir = tmp_path / "junk"
    excluded_dir.mkdir()
    kept_dir = tmp_path / "good"
    kept_dir.mkdir()
