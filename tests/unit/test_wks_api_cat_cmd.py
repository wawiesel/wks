...
@pytest.mark.cat
def test_cmd_cat_by_path(wks_home, minimal_config_dict):
    ...
    # cmd_cat by path
    res = run_cmd(cmd_cat, target=str(test_file))
...
@pytest.mark.cat
def test_cmd_cat_by_checksum(wks_home, minimal_config_dict):
    ...
    # 2. cmd_cat by checksum
    res = run_cmd(cmd_cat, target=checksum)
    assert res.success is True
    assert res.output["content"] == "Transformed: Checksum Cat"


@pytest.mark.cat
def test_cmd_cat_to_output_file(wks_home, minimal_config_dict):
    ...
    # cmd_cat with output_path
    res = run_cmd(cmd_cat, target=str(test_file), output_path=out_file)
    assert res.success is True
...
@pytest.mark.cat
def test_cmd_cat_nonexistent_file(wks_home):
    """Test cmd_cat with a missing file."""
    res = run_cmd(cmd_cat, target="/tmp/nonexistent_file_12345")
    assert res.success is False
    assert "File not found" in res.result


@pytest.mark.cat
def test_cmd_cat_nonexistent_checksum(wks_home):
    """Test cmd_cat with a missing checksum."""
    missing_checksum = "a" * 64
    res = run_cmd(cmd_cat, target=missing_checksum)
    assert res.success is False
    assert "Checksum not found in database" in res.result


@pytest.mark.cat
def test_cmd_cat_stale_cache_record(wks_home, minimal_config_dict):
    """Test cmd_cat when DB has record but file is missing from disk."""
    watch_dir = Path(wks_home).parent / "watched"
    watch_dir.mkdir(parents=True, exist_ok=True)
    ...
    # 3. cmd_cat should fail and clean up DB record
    res = run_cmd(cmd_cat, target=checksum)
    assert res.success is False
